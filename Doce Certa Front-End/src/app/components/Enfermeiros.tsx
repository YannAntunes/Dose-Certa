...
export default function Enfermeiros({ enfermeiros, onAddEnfermeiro, onUpdateEnfermeiro, onDeleteEnfermeiro }: EnfermeirosProps) {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <Card className="border-slate-200/60 dark:border-slate-800 bg-white/50 dark:bg-slate-900/50 backdrop-blur-xl shadow-sm">
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-slate-800 dark:text-slate-100">
            ...
            {t('profissionais.add_enfermeiro')}
          </CardTitle>
          <CardDescription className="dark:text-slate-400">{t('profissionais.add_enfermeiro_desc')}</CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="space-y-2">
                <Label htmlFor="coren" className="dark:text-slate-300">{t('profissionais.form.coren')}</Label>
                <Input id="coren" value={coren} onChange={e => setCoren(e.target.value)} placeholder="COREN" required className="bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700" />
              </div>
              <div className="space-y-2">
      ...
      <Card className="border-slate-200/60 dark:border-slate-800 bg-white/50 dark:bg-slate-900/50 backdrop-blur-xl shadow-sm">
        <CardHeader>
          <CardTitle className="text-slate-800 dark:text-slate-100">{t('profissionais.list.enfermeiros')}</CardTitle>
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-slate-50 dark:bg-slate-800/50 border-slate-200 dark:border-slate-800">
                  ...
                  <TableHead className="text-slate-600 dark:text-slate-300">{t('profissionais.form.coren')}</TableHead>
              ...
              <TableBody>
                {filteredEnfermeiros.length === 0 ? (
                  ...
                  filteredEnfermeiros.map(enf =>
                    editingId === enf.id ? (
                      <TableRow key={enf.id} className="bg-rose-50/50 dark:bg-rose-900/10 border-slate-200 dark:border-slate-800">
                        ...
                        <TableCell><Input value={editCoren} onChange={e => setEditCoren(e.target.value)} className="h-8 w-28 bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700" /></TableCell>
                        <TableCell>
                      ...
                      <TableRow key={enf.id} className="bg-red-50/50 dark:bg-red-900/10 border-slate-200 dark:border-slate-800">
                        <TableCell colSpan={3} className="text-sm text-red-700 dark:text-red-400 font-medium">
                          {t('common.delete')} <strong>{enf.nome}</strong>?
                        </TableCell>
                        <TableCell>
                          <div className="flex justify-end gap-1">
                            <Button size="sm" onClick={() => confirmDelete(enf.id)} className="bg-red-600 hover:bg-red-700 text-white h-8 px-2 text-xs">{t('common.delete')}</Button>
                      ...
                      <TableRow key={enf.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 border-slate-200 dark:border-slate-800">
                        <TableCell className="font-medium text-slate-800 dark:text-slate-200">{enf.nome}</TableCell>
                        <TableCell className="text-slate-700 dark:text-slate-300">{enf.coren}</TableCell>
                        <TableCell className="text-slate-700 dark:text-slate-300">{enf.estado}</TableCell>
                        <TableCell>
                          <div className="flex justify-end gap-1">
